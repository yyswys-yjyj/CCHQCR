...
@GetEventInfo(JSON->"code", $source)  // 对 JSON 字符串按路径提取
```

`JSON->"path"` 是 JSON 路径指示器，将数据源视为 JSON 字符串解析后按路径提取：

```cchq
@GetEventInfo(JSON->"name", $jsonStr)         // 解析 JSON 字符串提取 name
@GetEventInfo(JSON->"user.name", $jsonStr)     // 支持点路径嵌套
@GetEventInfo(JSON->"code", @GetEventInfo(RunFunc, result))  // 结合 RunFunc 结果
```
...
`@pick`（全称"捡取表达式"）用于从 Param 中提取值并进行模式匹配。
...
@pick(Param:$status){
    switch($status){
        case "code":"xxx"     // 等价于 case "code":{ $result="xxx";break; }
    }
}
```
...
@Regfunc<>Param:any&{
    ...
    if(1 == @GetEventInfo(Param, quantity)){
        @EventRestart(@pick(Param:$a));  // 从 Param 提取 $a 并重启函数
    }
}
```
